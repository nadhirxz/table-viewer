...
const Table = <U extends object>(props: Props<U>) => {
	...
	return (
		<div className={`w-full rounded-xl shadow-md p-5 flex-col border border-gray-200 dark:border-gray-850 ${props.containerClass ? props.containerClass : ''}`}>
			<div className={`table-auto ${props.className ?? ''}`}>
					...
					<tbody>
						{elements?.length ? (
							elements.slice((pages.current - 1) * pages.elementsPerPage, (pages.current - 1) * pages.elementsPerPage + pages.elementsPerPage).map((element, index) => (
								<tr className="hover:bg-gray-200 dark:hover:bg-gray-900 main-transition" key={element._id}>
										...
										key =>
											key != '_id' && (
												...
												<td key={key + index} className={`px-2 py-2 ${element[key].class || ''}`}>
													<div
														className={`break-words px-2 py-1 rounded-xl main-transition ${key == 'id' ? 'text-gray-500 font-bold text-xs' : ''} ${props.keysNotToCopy?.includes(key) ? '' : 'hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer active:bg-gray-300 dark:active:bg-gray-700'}`}
														// @ts-ignore
									...
									<td>
										<div className="group relative">
											{props.dropdownItems ? (
												<>
													<button className="round-icon hover:bg-gray-100 dark:hover:bg-gray-600 main-transition" onClick={toggleDropdown}>
														<More />
													</button>
												...
												<button className="round-icon hover:bg-gray-100 dark:hover:bg-gray-600 main-transition" onClick={() => props.dropdownReplacement?.onClick(parseInt(element._id))}>
				...
				<div className="mx-auto flex justify-center pt-3 border-t space-x-1 border-gray-700 mb-4">
					<div onClick={pages.current == 1 ? () => {} : () => setPages(prev => ({ ...prev, current: 1 }))} className={`round-icon hover:bg-gray-300 dark:hover:bg-gray-600 main-transition${pages.current == 1 ? ' opacity-30' : ''}`}>
						<PageFirst />
					</div>
					<div onClick={pages.current == 1 ? () => {} : () => setPages(prev => ({ ...prev, current: prev.current - 1 }))} className={`round-icon hover:bg-gray-300 dark:hover:bg-gray-600 main-transition${pages.current == 1 ? ' opacity-30' : ''}`}>
						<PagePrev />
					</div>
					{[...Array.from(Array(pages.number).keys())]
						...
						.map(e => (
							<div onClick={setCurrentPage} className={`round-icon hover:bg-gray-300 dark:hover:bg-gray-600 main-transition${e == pages.current ? ' bg-gray-300 dark:bg-gray-700' : ''}`} key={e}>
								{e}
							</div>
						))}
					<div onClick={pages.current == pages.number ? () => {} : () => setPages(prev => ({ ...prev, current: prev.current + 1 }))} className={`round-icon hover:bg-gray-300 dark:hover:bg-gray-600 main-transition${pages.current == pages.number ? ' opacity-30' : ''}`}>
						<PageNext />
					</div>
					<div onClick={pages.current == pages.number ? () => {} : () => setPages(prev => ({ ...prev, current: prev.number }))} className={`round-icon hover:bg-gray-300 dark:hover:bg-gray-600 main-transition${pages.current == pages.number ? ' opacity-30' : ''}`}>
						<PageLast />
					</div>
				</div>
				<Alert
